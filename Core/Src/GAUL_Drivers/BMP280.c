...
uint8_t BMP280_ReadPressure(BMP280 *BMP_data) {
    ...
    BMP_data->press_Pa = (float)p / (float)(1 << 8); // + devBMP->pressure_ref;
...
uint8_t BMP280_ReadAltitude(BMP280 *BMP_data) {
	...
	BMP_data->alt_m = BMP280_PressureToAltitude(BMP_data->press_Pa);
...
float BMP280_PressureToAltitude(float pressure) {
    ...
    return 44330 * (1.0 - pow(pressure / BMP280_PRESS_SEA_LEVEL, 0.1903));;
}
